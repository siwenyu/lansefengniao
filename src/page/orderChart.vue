        ...
        <div class="container main-body">
            <Row>
                ...
                <Col span="20" class="c-gap-inner-left main-content">
                    <Row class="main-content-form c-gap-inner-bottom c-gap-inner-left-large">
                        <Col span="24" >
                            <h4 class="c-line-bottom c-gap-inner-bottom">订单汇总图标</h4>
                        </Col>
                        <Col v-show="isAdmin" span="24">
                            <Row>
                                <Col span="24">
                                    <div>
                                        账户查询(pid)：
                    ...
                    <Row class="main-content-table">
                        <div class="echard-div c-gap-bottom-large c-gap-inner-bottom-large c-line-bottom">
                            <IEcharts id="chart1" :option="option"></IEcharts>
                        </div>
                        <div class="echard-div c-gap-bottom-large">
                            <IEcharts id="chart2" :option="option1"></IEcharts>
                        </div>
                        <div class="echard-div c-gap-bottom-large">
                            <IEcharts id="chart3" :option="option2"></IEcharts>
                        </div>
                    </Row>
                </Col>
            </Row>
...
export default {
    ...
    data () {
        return {
            ...
            option1 : {animation:true,
                ...
                grid: {},
                toolbox: {
                    feature: {saveAsImage: {},restore:{},dataView: {},dataZoom: {
						id: 'dataZoomX',
						type: 'slider',
						xAxisIndex: [0],
    ...
    methods:{
        fatchData(){
            let self = this;
            let url = self.ajaxUrl + "/commission/queryDetailOrders?";
        ...
        renderChart(data){
            ...
            for(let i = 0; i < data.list.length; i ++) {
				...
				if(data.list && data.list[0] && data.list[0].show_type == 0) {
					...
					this.option.xAxis.data.push(data.list[i].insertMonth);
				}
                this.option.series[0].data.push(tool.toDecimal2(data.list[i].orderNum));
                ...
                this.option2.series[0].data.push(tool.toDecimal2(data.list[i].real_commission));
            }
			// 按天查询的倒序
			if(data.list && data.list[0] && data.list[0].show_type == 0) {
