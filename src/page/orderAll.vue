            ...
            <Row>
                ...
                <Col span="20" class="c-gap-inner-left main-content">
                    <Row class="main-content-form c-gap-inner-bottom-large c-gap-inner-left-large">
                        <Col span="24" >
                            <h4 class="c-line-bottom c-gap-inner-bottom">订单汇总</h4>
                        </Col>
                        <Col span="24">
                            <Row>
                                <Col span="24">
...
export default {
    ...
    data () {
        return {
            ...
            duration: 33,
            isAdmin:false,
            allTrlist:['账户','姓名','时间','订单量','订单金额','结算佣金','预估佣金','用户比例'],
            itemTrlist:['账户','姓名','时间','订单量','订单金额','结算佣金'],
            tableList: {
                trList:[],
                tdList:[{
                    user_name: '',
    ...
    methods:{
        ...
        renderTabList(str){
            ...
            for(let i=0; i < str.length; i++){
                ...
                if(str[i].show_type == "1") {
                    this.tableList.tdList[i].insertMonth = str[i].insertMonth;
                } else {
                    this.tableList.tdList[i].insertTime = str[i].insertTime;
                }
                
                this.tableList.tdList[i].orderNum = str[i].orderNum;
                this.tableList.tdList[i].cosPriceSum = tool.toDecimal2(str[i].cosPriceSum);
                this.tableList.tdList[i].real_commission = tool.toDecimal2(str[i].real_commission) ? tool.toDecimal2(str[i].real_commission) : '暂无';
                if(this.isAdmin) {
